...
import live.thought.thoughtj.core.InventoryItem;
...
import live.thought.thoughtj.core.UTXOsMessage;
import live.thought.thoughtj.core.UnsafeByteArrayOutputStream;
...
import live.thought.thoughtj.core.Transaction.SigHash;
...
import static live.thought.thoughtj.script.ScriptOpCodes.*;
...
public class FullBlockTestGenerator {
    ...
    public RuleList getBlocksToTest(boolean runBarelyExpensiveTests, boolean runExpensiveTests, File blockStorageFile) throws ScriptException, ProtocolException, IOException {
        ...
        //                      \-> b3 (1) -> b4 (2)
        //
        NewBlock b9 = createNextBlock(b6, chainHeadHeight + 5, out4, NOTION);
        ...
        NewBlock b11 = createNextBlock(b10, chainHeadHeight + 5, out4, NOTION);
        ...
        NewBlock b14 = createNextBlock(b13, chainHeadHeight + 6, out5, NOTION);
        ...
        NewBlock b15 = createNextBlock(b13, chainHeadHeight + 6, out5, null);
        {
            ...
            tx.addOutput(new TransactionOutput(params, tx, NOTION, outputScript));
        ...
        {
            Transaction tx = new Transaction(params);
            tx.addOutput(new TransactionOutput(params, tx, NOTION, new byte[] {}));
        ...
        NewBlock b39 = createNextBlock(b35, chainHeadHeight + 12, null, null);
        {
            ...
            Coin lastOutputValue = out11.value.subtract(NOTION);
            TransactionOutPoint lastOutPoint;
            {
                Transaction tx = new Transaction(params);
                tx.addOutput(new TransactionOutput(params, tx, NOTION, scriptPubKey.toByteArray()));
            ...
            while (b39.block.getMessageSize() < Block.MAX_BLOCK_SIZE)
            {
                ...
                lastOutputValue = lastOutputValue.subtract(NOTION);
                tx.addOutput(new TransactionOutput(params, tx, NOTION, scriptPubKey.toByteArray()));
                tx.addOutput(new TransactionOutput(params, tx, lastOutputValue, new byte[]{OP_1}));
        ...
        NewBlock b40 = createNextBlock(b39, chainHeadHeight + 13, out12, null);
        {
            ...
            for (int i = 1; i <= numTxes; i++) {
                Transaction tx = new Transaction(params);
                tx.addOutput(new TransactionOutput(params, tx, NOTION, new byte[] {OP_1}));
                tx.addInput(new TransactionInput(params, tx, new byte[]{OP_1}, lastOutPoint));
        ...
        byte[] outScriptBytes = ScriptBuilder.createOutputScript(ECKey.fromPublicOnly(coinbaseOutKeyPubKey)).getProgram();
        {
            ...
            t.addOutput(new TransactionOutput(params, t, NOTION, outScriptBytes));
            // Spendable output
        ...
        NewBlock b51 = createNextBlock(b44, chainHeadHeight + 16, out15, null);
        {
            ...
            coinbase.addOutput(new TransactionOutput(params, coinbase, NOTION, outScriptBytes));
        ...
        Transaction b56txToDuplicate;
        {
            ...
            b56txToDuplicate.addOutput(new TransactionOutput(params, b56txToDuplicate, NOTION, new byte[] {}));
        ...
        Transaction b56p2txToDuplicate1, b56p2txToDuplicate2;
        {
            ...
            tx1.addOutput(new TransactionOutput(params, tx1, NOTION, new byte[] {OP_TRUE}));
            addOnlyInputToTransaction(tx1, b57p2);
            b57p2.addTransaction(tx1);

            Transaction tx2 = new Transaction(params);
            tx2.addOutput(new TransactionOutput(params, tx2, NOTION, new byte[] {OP_TRUE}));
            addOnlyInputToTransaction(tx2, new TransactionOutPointWithValue(
                    new TransactionOutPoint(params, 0, tx1.getHash()),
                    NOTION, tx1.getOutputs().get(0).getScriptPubKey()));
            b57p2.addTransaction(tx2);

            b56p2txToDuplicate1 = new Transaction(params);
            b56p2txToDuplicate1.addOutput(new TransactionOutput(params, b56p2txToDuplicate1, NOTION, new byte[]{OP_TRUE}));
            addOnlyInputToTransaction(b56p2txToDuplicate1, new TransactionOutPointWithValue(
                    ...
                    NOTION, tx2.getOutputs().get(0).getScriptPubKey()));
            b57p2.addTransaction(b56p2txToDuplicate1);

            b56p2txToDuplicate2 = new Transaction(params);
            b56p2txToDuplicate2.addOutput(new TransactionOutput(params, b56p2txToDuplicate2, NOTION, new byte[]{}));
            addOnlyInputToTransaction(b56p2txToDuplicate2, new TransactionOutPointWithValue(
                    ...
                    NOTION, b56p2txToDuplicate1.getOutputs().get(0).getScriptPubKey()));
        ...
        NewBlock b59 = createNextBlock(b57, chainHeadHeight + 18, out17, null);
        {
            ...
            tx.addOutput(new TransactionOutput(params, tx,
                    b59.getSpendableOutput().value.add(NOTION), new byte[]{}));
        ...
        //                                                                                    \-> b68 (20)
        //
        NewBlock b68 = createNextBlock(b65, chainHeadHeight + 21, null, NOTION.multiply(10));
        {
            Transaction tx = new Transaction(params);
        ...
        NewBlock b69 = createNextBlock(b65, chainHeadHeight + 21, null, NOTION.multiply(10));
        {
            Transaction tx = new Transaction(params);
    ...
    private NewBlock createNextBlock(Block baseBlock, int nextBlockHeight, @Nullable TransactionOutPointWithValue prevOut,
        ...
        Coin coinbaseValue = baseBlock.getBlockInflation(nextBlockHeight, baseBlock.getDifficultyTarget(), false)
                .add((prevOut != null ? prevOut.value.subtract(NOTION) : ZERO))
        ...
        if (prevOut != null) {
            ...
            // Spendable output
            t.addOutput(new TransactionOutput(params, t, NOTION, new byte[] {OP_1}));
