...
import live.thought.thoughtj.utils.ThtAutoFormat.Style;
...
import static live.thought.thoughtj.utils.ThtAutoFormat.Style.*;
 ...
 *       {@link ThtFormat#format(Object)} method. 
 *   <li>Parse a value by passing a {@code String}-type
 *       representation of it to the {@link ThtFormat#parse(String)} method.</ol>
 *
 * <p>For example, depending on your locale, values might be formatted
 * and parsed as follows:
 *
 * <blockquote><pre>
 * ThtFormat f = ThtFormat.getInstance();
 ...
 * be formatted, {@link ThtAutoFormat}, and another that formats any value in units of a
 * fixed, specified denomination, {@link ThtFixedFormat}.  
 *
 ...
 * values of {@link ThtAutoFormat.Style}.  There are two styles constants: {@link
 * ThtAutoFormat.Style#CODE} (the default), and {@link ThtAutoFormat.Style#SYMBOL}.  The
 ...
 *   <li>Use a {@link ThtFormat.Builder} object.</ol>
 *
 ...
 * methods of this abstract base class {@link ThtFormat}.  There
 ...
 * automatic-denominating {@link ThtAutoFormat} instance for your
 ...
 * the {@link ThtFixedFormat} instance that is returned, and thus will
 * determine its denomination.  For example, if you want to format
 * values in units of microbitcoins:
 *
 * <blockquote><pre>ThtFormat m = ThtFormat.getInstance(6);
 ...
 *
 * <blockquote><pre>ThtFormat milliFormat = ThtFormat.getInstance(MILLICOIN_SCALE);</pre></blockquote>
 *
 * <p>Alternatively, if the type of the first argument to
 * {@code getInstance()} is one of the enumerated values of the
 * {@link ThtAutoFormat.Style} type, either {@link ThtAutoFormat.Style#CODE} or
 * {@link ThtAutoFormat.Style#SYMBOL}, then you will get a {@link ThtAutoFormat}
 * instance that uses either a currency code or symbol, respectively,
 * to indicate the results of its auto-denomination.
 *
 * <blockquote><pre>
 * ThtFormat s = ThtFormat.getInstance(SYMBOL);
 ...
 * ThtFormat.getInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1,00 ฿"</strong>
 * ThtFormat.getInstance(SYMBOL, Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1.000,00"</strong>
 * ThtFormat.getMilliInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "10.000,00"</strong>
 * ThtFormat.getInstance(4, Locale.GERMANY).format(Coin.COIN);
 * </pre></blockquote>
 *
 ...
 *
 * A new {@link ThtFormat.Builder} instance is returned by the {@link #builder()} method.  Such
 * an object has methods that set the configuration parameters of a {@link ThtFormat}
 * object.  Its {@link Builder#build()} method constructs and returns a {@link ThtFormat} instance
 ...
 *
 * <blockquote><pre>
 * ThtFormat f = ThtFormat.builder().style(CODE).code("XDC").build();
 * String out = f.format(COIN); <strong>// "XDC 1.00"</strong>
 * </pre></blockquote>
 *
 * <p>See the documentation of the {@link ThtFormat.Builder} class for details.
 *
 * <h3>Formatting</h3>
 *
 * <p>You format a Bitcoin monetary value by passing it to the {@link ThtFormat#format(Object)}
 * method.  This argument can be either a {@link live.thought.thoughtj.core.Coin}-type object or a
 ...
 * {@link ThtFixedFormat} also accept a variable-length sequence of additional {@code int}
 ...
 *
 * <blockquote><pre>
 * ThtFormat f = ThtFormat.getCoinInstance();
 ...
 * {@link ThtFixedFormat#REPEATING_PLACES}, {@link
 * ThtFixedFormat#REPEATING_DOUBLETS} and {@link
 * ThtFixedFormat#REPEATING_TRIPLETS}.  These signify repeating groups
 ...
 *
 * <blockquote><pre>
 * ThtFormat de = ThtFormat.getInstance(Locale.GERMANY);
 ...
 *
 * <blockquote><pre>
 * ThtFixedFormat kilo = (ThtFixedFormat)ThtFormat(-3); <strong>// scale -3 for kilocoins</strong>
 ...
 * ThtFormat(4).code(); <strong>// unnamed denomination has no code; raises exception</strong>
 * </pre></blockquote>
 *
 ...
 *                SATOSHI};
 * ThtFormat f = ThtFormat.getCoinInstance(2, REPEATING_PLACES);
 ...
 * ThtFixedFormat} and {@link ThtAutoFormat} is analogous to the difference in formatting
 * behavior between instances of those classes.  Instances of {@link ThtAutoFormat} recognize
 ...
 * instances of {@link ThtFixedFormat} by default recognize no codes nor symbols, but rather
 ...
 * ThtFixedFormat} can be overridden by setting a parsing pattern that includes a currency sign
 * using the {@link ThtFormat.Builder#pattern()} method.
 *
 * <p>The {@link ThtAutoFormat#parse(String)}</code> method of {@link ThtAutoFormat} (and of
 * {@link ThtAutoFormat} configured with applicable non-default pattern) will recognize a
 ...
 * ThtFormat.Builder#code} or {@link ThtFormat.Builder#code}, then such code or symbol will
 ...
 * {@code ThtFormat} instance, on the other hand, whether or not the units indicator must
 * be separated by a space from the number is determined by the locale.  The {@link
 * ThtFormat#pattern()} method returns a representation of the pattern that
 * can be examined to determine whether a space must separate currency signs from numbers in
 * parsed {@link String}s.
 *
 * <p>When parsing, if the currency-units indicator is absent, then a {@link ThtAutoFormat}
 * instance will infer a denomination of bitcoins while a {@link ThtFixedFormat} will infer the
 ...
 * @see          live.thought.thoughtj.core.Coin
 */

public abstract class ThtFormat extends Format {
    ...
    /**
     * <p>This class constructs new instances of {@link ThtFormat}, allowing for the
     * configuration of those instances before they are constructed.  After obtaining a
     * {@code Builder} object from the {@link ThtFormat#builder()} method, invoke the
     * necessary setter methods to obtain your desired configuration.  Finaly, the {@link
     * #build()} method returns a new {@code ThtFormat} object that has the specified
     * configuration.
     *
     * <p>All the setter methods override defaults.  Invoking {@code build()} without invoking any
     * of the setting methods is equivalent to invoking {@link ThtFormat#getInstance()} with no arguments.
     *
    ...
    public static class Builder {
        ...
        private enum Variant {
            AUTO { @Override ThtFormat newInstance(Builder b) {
                       return getInstance(b.style, b.locale, b.minimumFractionDigits);
                   }},
            FIXED,
            UNSET;
            ThtFormat newInstance(Builder b) {
        ...
        private Style style = ThtAutoFormat.Style.CODE;
        private int scale = 0;
        private String symbol = "",code = "",pattern = "",localizedPattern = "";

        private Builder() {}

        /** Specify the new {@code ThtFormat} is to be automatically-denominating.
         * The argument determines which of either codes or symbols the new {@code ThtFormat}
        ...
        public Builder style(ThtAutoFormat.Style val) {
         ...
         * appropriately for the context of generating {@link ThtAutoFormat} instances.
         *
         ...
         *
         * <p>The {@code ThtFormat} class provides appropriately named
         * {@code int}-type constants for the three common values, {@link ThtFormat#COIN_SCALE},
         * {@link ThtFormat#MILLICOIN_SCALE} {@link ThtFormat#MICROCOIN_SCALE}.
         *
         * <p>If neither this method nor {@link #style(ThtAutoFormat.Style)} is invoked on a
         * {@link Builder}, then the {@link ThtFormat} will default to a
         ...
         *  class.
         *
         *  <p>The pattern is localized according to the locale of the {@code ThtFormat}
         *  instance, the symbols for which can be examined by inspecting the {@link
         *  java.text.DecimalFormatSymbols} object returned by {@link ThtFormat#symbols()}.
        ...
        /** Return a new {@link ThtFormat} instance.  The object returned will be configured according
         *  to the state of this {@code Builder} instance at the time this method is invoked. */
        public ThtFormat build() {
            ThtFormat f = variant.newInstance(this);
    ...
    protected ThtFormat(DecimalFormat numberFormat, int minDecimals, List<Integer> groups) {
     ...
     */
    public static ThtFormat getInstance() { return getInstance(defaultLocale()); }
     ...
     */
    public static ThtFormat getSymbolInstance() { return getSymbolInstance(defaultLocale()); }
     ...
     * according to the default locale.
     */
    public static ThtFormat getCodeInstance() { return getCodeInstance(defaultLocale()); }
     ...
     * locale.
     */
    public static ThtFormat getSymbolInstance(int fractionPlaces) {
     ...
     * locale.
     */
    public static ThtFormat getCodeInstance(int minDecimals) {
     ...
     */
    public static ThtFormat getInstance(Locale locale) { return getCodeInstance(locale); }
     ...
     * units using a currency code, for example, {@code "mBTC"}.
     */
    public static ThtFormat getCodeInstance(Locale locale) { return getInstance(CODE, locale); }
     ...
     * fractional satoshis.
     */
    public static ThtFormat getInstance(Locale locale, int minDecimals) {
     ...
     * fractional satoshis.
     */
    public static ThtFormat getCodeInstance(Locale locale, int minDecimals) {
     ...
     */
    public static ThtFormat getSymbolInstance(Locale locale) {
     ...
     * fractional satoshis.
     */
    public static ThtFormat getSymbolInstance(Locale locale, int fractionPlaces) {
     ...
     */
    public static ThtFormat getInstance(Style style) { return getInstance(style, defaultLocale()); }
     ...
     */
    public static ThtFormat getInstance(Style style, int fractionPlaces) {
     ...
     */
    public static ThtFormat getInstance(Style style, Locale locale) {
     ...
     * fractional satoshis.
     */
    public static ThtFormat getInstance(Style style, Locale locale, int fractionPlaces) {
	return new ThtAutoFormat(locale, style, fractionPlaces);
    }
     ...
     */
    public static ThtFormat getCoinInstance() { return getCoinInstance(defaultLocale()); }
     ...
     */
    public static ThtFormat getCoinInstance(int minFractionPlaces, int... groups) {
     ...
     */
    public static ThtFormat getCoinInstance(Locale locale) {
     ...
     */
    public static ThtFormat getCoinInstance(Locale locale, int scale, int... groups) {
     ...
     */
    public static ThtFormat getMilliInstance() { return getMilliInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static ThtFormat getMilliInstance(Locale locale) {
     ...
     */
    public static ThtFormat getMilliInstance(int scale, int... groups) {
     ...
     */
    public static ThtFormat getMilliInstance(Locale locale, int scale, int... groups) {
     ...
     * necessary.
     */
    public static ThtFormat getMicroInstance() { return getMicroInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static ThtFormat getMicroInstance(Locale locale) {
     ...
     * necessary to avoid rounding, to a limiting precision of satoshis.
     */
    public static ThtFormat getMicroInstance(int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static ThtFormat getMicroInstance(Locale locale, int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static ThtFormat getInstance(int scale, int minDecimals, int... groups) {
     ...
     */
    public static ThtFormat getInstance(int scale) {
     ...
     */
    public static ThtFormat getInstance(int scale, Locale locale) {
     ...
     */
    public static ThtFormat getInstance(int scale, Locale locale, int minDecimals, int... groups) {
     ...
     */
    public static ThtFormat getInstance(int scale, Locale locale, int minDecimals, List<Integer> groups) {
        return new ThtFixedFormat(locale, scale, minDecimals, groups);
    }
    ...
    /**
      ...
      * {@link live.thought.thoughtj.core.Coin} object that represents the parsed value.
      * @see java.text.NumberFormat */
    @Override
     ...
     * for instances of {@link ThtAutoFormat} and false for instances of {@link
     * ThtFixedFormat}, then denominated (i.e., prefixed) currency signs in the parsed String
     ...
     * ThtAutoFormat} will interpret the parsed number as a quantity of bitcoins, and instances
     * of {@link ThtAutoFormat} will interpret the number as a quantity of that instance's
     * configured denomination, which can be ascertained by invoking the {@link
     * ThtFixedFormat#symbol()} or {@link ThtFixedFormat#code()} method.
     *
    ...
    @Override public boolean equals(Object o) {
        ...
        if (!(o instanceof ThtFormat)) return false;
        ThtFormat other = (ThtFormat)o;
