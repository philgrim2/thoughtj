...
import live.thought.thoughtj.core.listeners.BlockChainListener;
import live.thought.thoughtj.core.listeners.NewBestBlockListener;
import live.thought.thoughtj.evolution.EvolutionUserManager;
import live.thought.thoughtj.evolution.SimplifiedMasternodeListManager;
import live.thought.thoughtj.governance.GovernanceManager;
import live.thought.thoughtj.governance.GovernanceTriggerManager;
import live.thought.thoughtj.governance.VoteConfidenceTable;
import live.thought.thoughtj.store.FlatDB;
import live.thought.thoughtj.store.HashStore;
 ...
 * scoped to a specific instantiation of thoughtj for a specific network. You
 * can get an instance of this class through calling {@link #get()}.
 * </p>
 *
 * <p>
 ...
 * instances of the library are in use simultaneously.
 * </p>
 */
public class Context
{
   ...
   * framework. Eventually you will be expected to do this yourself in the same
   * manner as fetching a NetworkParameters object (at the start of your app).
   *
   * @param params
  ...
  {
    log.info("Creating thoughtj {} context.", VersionMessage.THOUGHTJ_VERSION);
    this.confidenceTable = new TxConfidenceTable();
    this.voteConfidenceTable = new VoteConfidenceTable();
    this.params = params;
    lastConstructed = this;
    // We may already have a context in our TLS slot. This can happen a lot during
    // unit tests, so just ignore it.
    slot.set(this);
  }
   ...
   *          The network parameters that will be associated with this context.
   * @param eventHorizon
   *          Number of blocks after which the library will delete data and be
   *          unable to always process reorgs (see {@link #getEventHorizon()}.
   * @param feePerKb
   *          The default fee per 1000 bytes of transaction data to pay when
   *          completing transactions. For details, see
   *          {@link SendRequest#feePerKb}.
   * @param ensureMinRequiredFee
   *          Whether to ensure the minimum required fee by default when
   *          completing transactions. For details, see
   ...
   * thread</b>. ThoughtJ is an API that has thread affinity: much like OpenGL it
   * expects each thread that accesses it to have been configured with a global
   * Context object. This method returns that. Note that to help you develop, this
   * method will <i>also</i> propagate whichever context was created last onto the
   * current thread, if it's missing. However it will print an error when doing so
   * because propagation of contexts is meant to be done manually: this is so two
   * libraries or subsystems that independently use thoughtj (or possibly alt coin
   * forks of it) can operate correctly.
   *
   * @throws java.lang.IllegalStateException
   *           if no context exists at all or if we are in strict mode and there
   *           is no context.
   */
  public static Context get()
  {
    ...
    if (tls == null)
    {
      if (isStrictMode)
      {
        log.error("Thread is missing a thoughtj context.");
        log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
        throw new IllegalStateException("missing context");
      }
      if (lastConstructed == null)
        throw new IllegalStateException("You must construct a Context object before using thoughtj!");
      slot.set(lastConstructed);
      log.error("Performing thread fixup: you are accessing thoughtj via a thread that has not had any context set on it.");
   ...
   * {@link live.thought.thoughtj.utils.ContextPropagatingThreadFactory}, rather
   * than using a heuristic for the desired context.
   */
   ...
   * you create your own threads that want to create core ThoughtJ objects.
   * Generally, if a class can accept a Context in its constructor and might be
   * used (even indirectly) by a thread, you will want to call this first. Your
   * task may be simplified by using a
   * {@link live.thought.thoughtj.utils.ContextPropagatingThreadFactory}.
   */
   ...
   * measured as a proportion of how many peers announced it. With an un-tampered
   * with internet connection, the more peers announce a transaction the more
   * confidence you can have that it's really valid.
   */
  ...
  /**
   * Returns the {@link live.thought.thoughtj.core.NetworkParameters} specified
   * when this context was (auto) created. The network parameters defines various
   * hard coded constants for a specific instance of a Bitcoin network, such as
   ...
   * library consider a transaction to be so confirmed that it's safe to delete
   * data. Re-orgs larger than the event horizon will not be correctly processed,
  ...
  {
    this.liteMode = liteMode;// liteMode; --TODO: currently only lite mode has been tested and works with
                             // 12.1
  ...
  public void updatedChainHead(StoredBlock chainHead)
  {
    ...
    if (initializedDash)
    {
      ...
      /*
       * darkSendPool.UpdatedBlockTip(pindex); instantsend.UpdatedBlockTip(pindex);
       * mnpayments.UpdatedBlockTip(pindex); governance.UpdatedBlockTip(pindex);
