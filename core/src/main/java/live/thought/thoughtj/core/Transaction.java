...
import live.thought.thoughtj.script.ScriptOpCodes;
...
import static live.thought.thoughtj.core.Utils.*;
...
public class Transaction extends ChildMessage {
    ...
    public static final int MAX_STANDARD_TX_SIZE = 200000; // ajh - max transaction size dip001 increase
    ...
    /**
     * Used by ThoughtSerializer.  The serializer has to calculate a hash for checksumming so to
    ...
    /**
     * Same as {@link #addSignedInput(TransactionOutPoint, live.thought.thoughtj.script.Script, ECKey, live.thought.thoughtj.core.Transaction.SigHash, boolean)}
     ...
     * a wrapper around calling {@link Transaction#hashForSignature(int, byte[], live.thought.thoughtj.core.Transaction.SigHash, boolean)}
        ...
        try {
            ...
            if ((sigHashType & 0x1f) == SigHash.NONE.value) {
                ...
                for (int i = 0; i < inputIndex; i++)
                    tx.outputs.set(i, new TransactionOutput(tx.params, tx, Coin.NEGATIVE_NOTION, new byte[] {}));
                // The signature isn't broken by new versions of the transaction issued by other parties.
                for (int i = 0; i < tx.inputs.size(); i++)
                    if (i != inputIndex)
                        tx.inputs.get(i).setSequenceNumber(0);
    ...
    /**
     * Returns the confidence object for this transaction from the {@link live.thought.thoughtj.core.TxConfidenceTable}
