...
import live.thought.thoughtj.net.AbstractTimeoutHandler;
import live.thought.thoughtj.net.MessageWriteTarget;
 ...
 * {@code live.thought.thoughtj.net} classes and {@link Peer}.
 */
public abstract class PeerSocketHandler extends AbstractTimeoutHandler implements StreamConnection {
    ...
    private ThoughtSerializer.BitcoinPacketHeader header;
    ...
    public int receiveBytes(ByteBuffer buff) {
        checkArgument(buff.position() == 0 &&
                buff.capacity() >= ThoughtSerializer.BitcoinPacketHeader.HEADER_LENGTH + 4);
        try {
