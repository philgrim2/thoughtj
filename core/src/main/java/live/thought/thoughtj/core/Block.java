...
import static live.thought.thoughtj.core.Sha256Hash.*;
...
{
  /**
   * Flags used to control which elements of block validation are done on received
   * blocks.
   */
  ...
  /**
   * A value for difficultyTarget (nBits) that allows half of all possible hash
   * solutions. Used in unit testing.
   */
  ...
  // Blocks can be encoded in a way that will use more bytes than is optimal (due
  // to VarInts having multiple encodings)
  ...
  /**
   * Special case constructor, used for the genesis node, cloneAsHeader and unit
   * tests.
   */
   ...
   * 
   * @deprecated Use {@link ThoughtSerializer#makeBlock(byte[])} instead.
   */
  @Deprecated
   ...
   *          deserializing of the wire as the length will be provided as part of
   *          the header. If unknown then set to Message.UNKNOWN_LENGTH
   ...
   *          The location of the first payload byte within the array.
   * @param parent
   *          The message element which contains this block, maybe null for no
   *          parent.
   * @param serializer
  ...
  /**
   * <p>
   * A utility method that calculates how much new Bitcoin would be created by the
   * block at the given height. The inflation of Bitcoin is predictable and drops
   * roughly every 4 years (210,000 blocks). At the dawn of the system it was 50
   * coins per block, in late 2012 it went to 25 coins per block, and so on. The
   * size of a coinbase transaction is inflation plus fees.
   * </p>
   *
   * <p>
   * The half-life is controlled by
   * {@link live.thought.thoughtj.core.NetworkParameters#getSubsidyDecreaseBlockCount()}.
   * </p>
   */
   ...
   * 
   * public static Coin getBlockInflation(NetworkParameters params, int height,
   * long nPrevBits, boolean fSuperblockPartOnly) { double dDiff; long
   * nSubsidyBase; int nPrevHeight = height - 1;
   * 
   * if (nPrevHeight <= 4500 &&
   * params.getId().equals(NetworkParameters.ID_MAINNET)) { // a bug which caused
   * diff to not be correctly calculated dDiff = (double)0x0000ffff /
   * (double)(nPrevBits & 0x00ffffff); } else { dDiff =
   * Utils.convertBitsToDouble(nPrevBits); }
   * 
   * if (nPrevHeight < 5465) { // Early ages... // 1111/((x+1)^2) nSubsidyBase =
   * (long)(1111.0 / (Math.pow((dDiff+1.0),2.0))); if(nSubsidyBase > 500)
   * nSubsidyBase = 500; else if(nSubsidyBase < 1) nSubsidyBase = 1; } else if
   * (nPrevHeight < 17000 || (dDiff <= 75 && nPrevHeight < 24000)) { // CPU mining
   * era // 11111/(((x+51)/6)^2) nSubsidyBase = (long)(11111.0 /
   * (Math.pow((dDiff+51.0)/6.0,2.0))); if(nSubsidyBase > 500) nSubsidyBase = 500;
   * else if(nSubsidyBase < 25) nSubsidyBase = 25; } else { // GPU/ASIC mining era
   * // 2222222/(((x+2600)/9)^2) nSubsidyBase = (long)(2222222.0 /
   * (Math.pow((dDiff+2600.0)/9.0,2.0))); if(nSubsidyBase > 25) nSubsidyBase = 25;
   * else if(nSubsidyBase < 5) nSubsidyBase = 5; }
   * 
   * // LogPrintf("height %u diff %4.2f reward %d\n", nPrevHeight, dDiff,
   * nSubsidyBase); Coin nSubsidy = Coin.valueOf(nSubsidyBase * 100000000);
   * 
   * // yearly decline of production by ~7.1% per year, projected ~18M coins max
   * by year 2050+. for (int i = params.getSubsidyDecreaseBlockCount(); i <=
   * nPrevHeight; i += params.getSubsidyDecreaseBlockCount()) { nSubsidy =
   * nSubsidy.subtract(nSubsidy.div(14)); }
   * 
   * // Hard fork to reduce the block reward by 10 extra percent (allowing
   * budget/superblocks) Coin nSuperblockPart = (nPrevHeight >
   * params.getBudgetPaymentsStartBlock()) ? nSubsidy.div(10) : Coin.ZERO;
   * 
   * return fSuperblockPartOnly ? nSuperblockPart :
   * nSubsidy.minus(nSuperblockPart); }
   */
   ...
   *          Offset of the transactions within the block. Useful for non-Bitcoin
   *          chains where the block header may not be a fixed size.
   */
  ...
  public byte[] bitcoinSerialize()
  {
    ...
    // Preconditions.checkNotNull(payload, "Bytes should never be null if
    // headerBytesValid && transactionBytesValid");
   ...
   * cases where not will probably slightly oversize.
   *
   * This is used to preallocate the underlying byte array for a
   * ByteArrayOutputStream. If the size is under the real value the only penalty
   * is resizing of the underlying byte array.
   */
  ...
  {
    // Since we have alternate uncache methods to use internally this will only ever
    // be called by a child
  ...
  private void unCacheTransactions()
  {
    ...
    if (!headerBytesValid)
      payload = null;
    // Current implementation has to uncache headers as well as any change to a tx
    // will alter the merkle root. In
    // future we can go more granular and cache merkle root separately so rest of
    // the header does not need to be
    // rewritten.
    unCacheHeader();
  ...
  /**
   * <p>
   * Finds a value of nonce that makes the blocks hash lower than the difficulty
   * target. This is called mining, but solve() is far too slow to do real mining
   * with. It exists only for unit testing purposes.
   *
   * <p>
   * This can loop forever if a solution cannot be found solely by incrementing
   * nonce. It doesn't change extraNonce.
   * </p>
   */
  public void solve()
  {
  ...
  private void checkTimestamp() throws VerificationException
  {
    ...
    if (time > currentTime + ALLOWED_TIME_DRIFT)
      throw new VerificationException(
          String.format(Locale.US, "Block too far in future: %d vs %d", time, currentTime + ALLOWED_TIME_DRIFT));
  }

  private void checkSigOps() throws VerificationException
  {
    // Check there aren't too many signature verifications in the block. This is an
    // anti-DoS measure, see the
  ...
  private List<byte[]> buildMerkleTree()
  {
    ...
    // into a block without having to
    // provide the full block contents. Instead, you can provide only a Merkle
    // branch. For example to prove tx2 was
    // in a block you can just provide tx2, the hash(tx1) and B. Now the other party
    // has everything they need to
    // derive the root, which can be checked against the block header. These proofs
    // aren't used right now but
  ...
  {
    // Prove that this block is OK. It might seem that we can just ignore most of
    // these checks given that the
    // network is also verifying the blocks, but we cannot as it'd open us to a
    // variety of obscure attacks.
    //
    // Firstly we need to ensure this block does in fact represent real work done.
    ...
    {
      if (getPrevBlockHash().equals(params.genesisBlock.getHash()) && !getHash().equals(params.getDevNetGenesisBlock().getHash()))
      {
   ...
   *
   * @param height
   *          block height, if known, or -1 otherwise. If valid, used to validate
   *          the coinbase input script of v2 and above blocks.
   * @param flags
   *          flags to indicate which tests should be applied (i.e. whether to
   *          test for height in the coinbase transaction).
   * @throws VerificationException
   *           if there was an error verifying the block.
   */
  public void verifyTransactions(final int height, final EnumSet<VerifyFlag> flags) throws VerificationException
  {
    // Now we need to check that the body of the block actually matches the headers.
    // The network won't generate
    // an invalid block, but if we didn't validate this then an untrusted
    // man-in-the-middle could obtain the next
    // valid block from the network and simply replace the transactions in it with
    // their own fictional
  ...
  /**
   * Returns the hash of the previous block in the chain, as defined by the block
   * header.
   */
   ...
   * encoded <b>in compact form</b>. The {@link BlockChain} verifies that this is
   * not too easy by looking at the length of the chain when the block is added.
   * To find the actual value the hash should be compared against, use
   * {@link live.thought.thoughtj.core.Block#getDifficultyTargetAsInteger()}. Note
   * that this is <b>not</b> the same as the difficulty value reported by the
   * Bitcoin "getdifficulty" RPC that you may see on various block explorers. That
   * number is the result of applying a formula to the underlying difficulty to
   * normalize the minimum to 1. Calculating the difficulty that way is currently
   * unsupported.
   */
  ...
  /**
   * Returns a solved block that builds on top of this one. This exists for unit
   * tests.
   */
  @VisibleForTesting
  ...
  /**
   * Returns a solved block that builds on top of this one. This exists for unit
   * tests. In this variant you can specify a public key (pubkey) for use in
   * generating coinbase blocks.
   *
   * @param height
   *          block height, if known, or -1 otherwise.
   */
  Block createNextBlock(@Nullable final Address to, final long version, @Nullable TransactionOutPoint prevOut, final long time,
      final byte[] pubKey, final Coin coinbaseValue, final int height)
  {
    ...
    if (to != null)
    {
      ...
      // The input does not really need to be a valid signature, as long as it has the
      // right general form.
      TransactionInput input;
      if (prevOut == null)
      {
        input = new TransactionInput(params, t, Script.createInputScript(EMPTY_BYTES, EMPTY_BYTES));
        // Importantly the outpoint hash cannot be zero as that's how we detect a
        // coinbase transaction in isolation
