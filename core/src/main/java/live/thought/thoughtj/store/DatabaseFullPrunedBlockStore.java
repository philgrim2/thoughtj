...
public abstract class DatabaseFullPrunedBlockStore implements FullPrunedBlockStore {
        ...
        for(String sql : getCompatibilitySQL()) {
            ...
            try {
                ...
                throw new BlockStoreException("Database block store is not compatible with the current release.  " +
                        "See thoughtj release notes for further information: " + ex.getMessage());
            } finally {
                if (ps != null && !ps.isClosed()) {
                    ps.close();
                }
    ...
    /**
     * Create a new store for the given {@link live.thought.thoughtj.core.NetworkParameters}.
     ...
     *
     * <p>The balance {@link live.thought.thoughtj.store.DatabaseFullPrunedBlockStore#getBalanceSelectSQL()} returns
