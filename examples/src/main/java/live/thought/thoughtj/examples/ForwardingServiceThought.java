...
import live.thought.thoughtj.evolution.*;
import live.thought.thoughtj.governance.GovernanceManager;
import live.thought.thoughtj.governance.GovernanceObject;
import live.thought.thoughtj.governance.GovernanceVoteBroadcast;
import live.thought.thoughtj.kits.EvolutionWalletAppKit;
import live.thought.thoughtj.kits.WalletAppKit;
import live.thought.thoughtj.masternode.owner.MasternodeControl;
import live.thought.thoughtj.params.DevNetParams;
 ...
 */
public class ForwardingServiceThought
{
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    if (args.length < 1)
    {
      System.err.println("Usage: address-to-send-back-to");
      return;
    }
    ...
    Context.get().masternodeSync.addEventListener(new MasternodeSyncListener()
    {
      ...
      public void onSyncStatusChanged(int newStatus, double syncStatus)
      {
        if (newStatus == MasternodeSync.MASTERNODE_SYNC_FINISHED)
        {
          ...
          /*
           * Address address = new Address(params, params.getAddressHeader(), new
           * DumpedPrivateKey(params,
           * "91hMFoqbdqJdmCfgSniMBsWP4Vy9HDbdujg5Y7vpqCMXafw49jt").getKey().getPubKeyHash
           * ()); int version = address.getVersion();
           * 
           * byte [] ip = {13, (byte)229, 80, 108}; try { Masternode mn =
           * Context.get().masternodeManager.find(new
           * MasternodeAddress(InetAddress.getByAddress("13.229.80.108", ip), 19999)); if
           * (mn == null) {
           * 
          ...
          ArrayList<GovernanceObject> list = Context.get().governanceManager
              .getAllNewerThan(Utils.currentTimeSeconds() - 2 * 30 * 24 * 60 * 60);
          // ArrayList<Pair<Integer, Masternode>> results =
          // Context.get().masternodeManager.getMasternodeRanks(27143, 0);
          ...
          for (GovernanceObject governanceObject : list)
          {
            ...
            {
              GovernanceVoteBroadcast broadcast = control.voteAlias("test_mn", governanceObject.getHash().toString(), "funding",
                  "yes", error);
            }
          }
        }
  ...
  private static void createUser(Transaction tx)
  {
    try
    {
      ...
      privKey = ECKey.fromPrivate(
          kit.wallet().getActiveKeyChain().getKeyByPath(DeterministicKeyChain.ACCOUNT_ZERO_PATH, false).getPrivKeyBytes());
      newPrivKey = ECKey.fromPrivate(
          kit.wallet().getActiveKeyChain().getKeyByPath(ImmutableList.of(ChildNumber.ONE_HARDENED), true).getPrivKeyBytes());
      SendRequest req = SendRequest.forSubTxRegister(kit.params(),
          new SubTxRegister(1, "hashengineering" + new Random().nextInt() / 1000, privKey), amount);
      ...
      FlatDB<EvolutionUserManager> mndb = new FlatDB<EvolutionUserManager>(kit.directory().getAbsolutePath(), "user.dat",
          "magicMasternodeCache");
  ...
  private static void topup(Transaction tx)
  {
    try
    {
      ...
      SendRequest topup = SendRequest.forSubTxTopup(kit.params(), new SubTxTopup(1, currentUser.getRegTxId()),
          Coin.valueOf(50000000));
  ...
  private static void reset(Transaction tx)
  {
    try
    {
      ...
      SendRequest reset = SendRequest.forSubTxResetKey(kit.params(), new SubTxResetKey(1, currentUser.getRegTxId(),
          currentUser.getCurSubTx(), SubTxTransition.EVO_TS_MIN_FEE, new KeyId(newPrivKey.getPubKeyHash()), privKey));
  ...
  private static void forwardCoins(Transaction tx)
  {
    try
    {
      ...
      // confirmation.
      // final Coin amountToSend =
      // value.subtract(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE);
      ...
      FlatDB<MasternodeManager> mndb = new FlatDB<MasternodeManager>(kit.directory().getAbsolutePath(), "mncache.dat",
          "magicMasternodeCache");
      mndb.dump(Context.get().masternodeManager);
    }
    catch (KeyCrypterException | InsufficientMoneyException e)
