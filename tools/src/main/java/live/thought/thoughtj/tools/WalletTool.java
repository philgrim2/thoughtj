...
package live.thought.thoughtj.tools;
...
import live.thought.thoughtj.core.CheckpointManager;
...
import live.thought.thoughtj.crypto.MnemonicCode;
import live.thought.thoughtj.crypto.MnemonicException;
...
public class WalletTool {
    ...
    private static live.thought.thoughtj.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    private static void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith(CoinDefinition.coinURIScheme)) {
            ...
            try {
                paymentRequest = live.thought.thoughtj.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
